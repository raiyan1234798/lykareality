...
export default function SupportCenter() {
    ...
    // Admin sees all tickets; user sees only their own single thread
    const [tickets, setTickets] = useState<Ticket[]>([
        {
            ...
            messages: [
                ...
                { id: 102, text: "Hi Sarah, I've checked your account and unlocked the module for you. Please try again.", sender: "admin", time: "10:30 AM" }
            ]
        },
        {
            id: 2,
    ...
    // User's own support thread with admin
    const [userMessages, setUserMessages] = useState<Message[]>([
        { id: 301, text: "Welcome to Lyka Academy Support! How can we help you today?", sender: "admin", time: "Today" }
    ]);
    ...
    const handleAdminSend = (e: React.FormEvent) => {
        ...
        setTickets(tickets.map(tk => {
            if (tk.id === activeTicketId) {
                return { ...tk, preview: reply, messages: [...tk.messages, { id: Date.now(), text: reply, sender: "admin", time: "Just now" }] };
            }
            return tk;
        }));
        setReply("");
    ...
    const handleUserSend = (e: React.FormEvent) => {
        ...
        setUserMessages([...userMessages, { id: Date.now(), text: reply, sender: "user", time: "Just now" }]);
        setReply("");
        // Simulate admin auto-reply after a moment
        setTimeout(() => {
            setUserMessages(prev => [...prev, { id: Date.now() + 1, text: t("Thank you for your message. An admin will get back to you shortly."), sender: "admin", time: "Just now" }]);
        }, 1500);
    };
    ...
    const handleNewTicket = (e: React.FormEvent) => {
        ...
        setUserMessages([...userMessages, { id: Date.now(), text: newTicketMessage, sender: "user", time: "Just now" }]);
    ...
    const selectTicket = (id: number) => {
        ...
        setTickets(tickets.map(tk => tk.id === id ? { ...tk, unread: false } : tk));
    };

    // ═══════════════════════════════════════════════
    // USER VIEW — single conversation with admin only
    // ═══════════════════════════════════════════════
    if (!isAdmin) {
        return (
            <div className="space-y-6 h-full flex flex-col">
                ...
                <div className="flex-1 min-h-[600px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm flex flex-col backdrop-blur-md">
                    ...
                    {/* Input */}
                    <form onSubmit={handleUserSend} className="p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/50">
                        <div className="relative flex items-center">
                            ...
                            <button type="submit" disabled={!reply.trim()} className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-violet-600 text-white rounded-full flex items-center justify-center hover:bg-violet-700 disabled:opacity-50 transition-colors shadow-md">
                ...
                <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/5 flex flex-col bg-slate-50 dark:bg-black/20">
                    <div className="p-4 border-b border-slate-200 dark:border-white/5">
                        <div className="relative">
                            ...
                            <input type="text" placeholder={t("Search consultants...")} className="w-full bg-slate-100 dark:bg-black/40 border border-slate-300 dark:border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-violet-500" />
                        </div>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {tickets.map((ticket) => (
                            <div key={ticket.id} onClick={() => selectTicket(ticket.id)} className={`p-4 border-b border-slate-100 dark:border-white/5 cursor-pointer transition-colors ${activeTicketId === ticket.id ? 'bg-slate-200 dark:bg-white/[0.04]' : 'hover:bg-slate-100 dark:hover:bg-white/[0.02]'}`}>
                ...
                <div className="flex-1 flex flex-col bg-white dark:bg-transparent">
                    {activeTicket ? (
                        <>
                            ...
                            <form onSubmit={handleAdminSend} className="p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/50">
                                <div className="relative flex items-center">
