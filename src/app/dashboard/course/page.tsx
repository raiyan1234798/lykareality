            ...
            <div className="flex-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2">
                <div className="relative aspect-video bg-black rounded-2xl overflow-hidden border border-white/5 shadow-2xl group">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <button
                            ...
                            className="w-20 h-20 bg-gold-500/90 text-blue-950 rounded-full flex items-center justify-center hover:scale-110 hover:bg-gold-400 transition-all shadow-[0_0_40px_rgba(212,175,55,0.4)] pointer-events-auto backdrop-blur-md"
                        >
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/80 to-transparent flex flex-col gap-4">
                        <div className="w-full h-1.5 bg-white/20 rounded-full overflow-hidden cursor-pointer relative group/progress">
                            <div className="absolute top-0 left-0 h-full w-[35%] bg-gold-500" />
                        ...
                        <div className="flex justify-between items-center text-white">
                            <div className="flex items-center gap-4 text-sm">
                                <button className="hover:text-gold-500 transition-colors pointer-events-auto" onClick={() => setIsPlaying(!isPlaying)}>
                            ...
                            <div className="flex items-center gap-4 text-zinc-400 pointer-events-auto">
                                <button className="text-xs font-semibold px-2 border border-white/20 rounded hover:border-gold-500 hover:text-gold-500 transition-colors">1x</button>
                ...
                <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-2xl p-6 md:p-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        <div>
                            <div className="text-gold-500 font-medium tracking-wide text-xs uppercase mb-2">Module 1 • Lesson 2</div>
                    ...
                    <div className="flex gap-6 border-b border-white/10 mb-6">
                        {["overview", "transcript", "notes"].map(tab => (
                            <button
                                ...
                                className={`py-3 text-sm font-medium capitalize relative ${activeTab === tab ? "text-gold-500" : "text-zinc-500 hover:text-zinc-300"}`}
                            >
                                {tab}
                                {activeTab === tab && (
                                    <motion.div layoutId="tab-active" className="absolute bottom-0 left-0 w-full h-0.5 bg-gold-500" />
                                )}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="text-zinc-400 text-sm leading-relaxed font-light min-h-[150px]">
                        ...
                        {activeTab === "transcript" && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                                ...
                                <p><span className="text-gold-500 font-medium">04:22 - </span> The investor doesn't care about the marble finish. They care about who else lives in the building.</p>
                            </motion.div>
                        )}
                        {activeTab === "notes" && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-4">
                                <textarea
                                    className="w-full bg-black/50 border border-white/10 rounded-lg p-4 text-white placeholder-zinc-600 focus:outline-none focus:border-gold-500 text-sm resize-none min-h-[100px]"
                                    placeholder="Private notes tied to this timestamp (04:22)..."
                                />
                                <div className="flex justify-end">
                                    <button className="px-4 py-2 bg-blue-950 border border-white/10 hover:border-gold-500 text-white rounded text-xs transition-colors">Save Note</button>
                                </div>
                            </motion.div>
                        )}
                    </div>
                ...
                <div className="p-6 border-b border-white/5 bg-black/40">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                            <div className="h-full w-[25%] bg-gold-500 rounded-full" />
                        </div>
                        <span className="text-gold-500 text-xs font-bold">25%</span>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                    {MODULES.map((module) => (
                        <div key={module.id} className="bg-black/20 rounded-xl overflow-hidden border border-white/5">
                            ...
                            <div className="px-2 pb-2">
                                {module.lessons.map(lesson => (
                                    <div
                                        key={lesson.id}
                                        className={`flex items-start gap-3 p-3 rounded-lg border border-transparent transition-all mb-1 ${lesson.active ? "bg-blue-900/30 border-blue-500/30" :
                                            lesson.completed ? "hover:bg-white/5 cursor-pointer" :
                                                "opacity-50 cursor-not-allowed bg-black/50"
                                            }`}
                                    >
                                        <div className="shrink-0 mt-0.5">
                                            {lesson.active ? <PlayCircle className="w-5 h-5 text-gold-500 animate-pulse" /> :
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className={`text-sm truncate font-medium ${lesson.active ? "text-gold-500" : "text-zinc-300"}`}>{lesson.title}</div>
